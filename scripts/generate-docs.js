...
function generateMarkdownTable(data) {
	...
	// Add separator row
	table +=
		'|' + data[0].map((cell) => '-'.repeat(cell.length)).join('|') + '|\n';
...
void (async () => {
	...
	for (const contractKey of Object.keys(baseImageContracts)) {
		await fs.appendFile(
			`${DOCS_DIRECTORY}/base-images-ref.md`,
			`## ${contractKey}\n`,
		);
		...
		for (const baseImageContract of baseImageContracts[contractKey]) {
			baseImagesTable.push([
				baseImageContract.imageName,
				`[Docker Hub](https://hub.docker.com/r/balenalib/${baseImageContract.imageName}), [GitHub](https://github.com/balena-io-library/base-images/tree/master/balena-base-images/device-base/${baseImageContract.slug})`,
			]);
		}

		await fs.appendFile(
			`${DOCS_DIRECTORY}/base-images-ref.md`,
			generateMarkdownTable(baseImagesTable) + '\n',
		);
	...
	console.log(
		`Base images table generated: ${DOCS_DIRECTORY}/base-images-ref.md`,
	);
})();
