...
```dockerfile
FROM balenalib/nitrogen6xq2g-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nitrogen6xq2g-ubuntu-openjdk:latest javac Main.java
```
