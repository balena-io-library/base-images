...
```dockerfile
FROM balenalib/n310-tx2-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/n310-tx2-alpine-openjdk:latest javac Main.java
```
