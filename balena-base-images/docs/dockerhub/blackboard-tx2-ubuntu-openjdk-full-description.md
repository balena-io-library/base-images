...
```dockerfile
FROM balenalib/blackboard-tx2-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/blackboard-tx2-ubuntu-openjdk:latest javac Main.java
```
