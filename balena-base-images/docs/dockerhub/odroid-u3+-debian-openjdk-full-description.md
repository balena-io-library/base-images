...
```dockerfile
FROM balenalib/odroid-u3+-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/odroid-u3+-debian-openjdk:latest javac Main.java
```
