...
```dockerfile
FROM balenalib/raspberrypi5-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypi5-ubuntu-openjdk:latest javac Main.java
```
