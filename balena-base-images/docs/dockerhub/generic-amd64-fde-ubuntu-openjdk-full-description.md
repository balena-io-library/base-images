...
```dockerfile
FROM balenalib/generic-amd64-fde-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/generic-amd64-fde-ubuntu-openjdk:latest javac Main.java
```
