...
```dockerfile
FROM balenalib/ucm-imx93-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/ucm-imx93-debian-openjdk:latest javac Main.java
```
