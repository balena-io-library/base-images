...
```dockerfile
FROM balenalib/kitra520-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/kitra520-fedora-openjdk:latest javac Main.java
```
