...
```dockerfile
FROM balenalib/am571x-evm-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/am571x-evm-fedora-openjdk:latest javac Main.java
```
