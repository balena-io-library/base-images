...
```dockerfile
FROM balenalib/solidrun-n6g-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/solidrun-n6g-alpine-openjdk:latest javac Main.java
```
