...
```dockerfile
FROM balenalib/jetson-nano-2gb-devkit-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-nano-2gb-devkit-fedora-openjdk:latest javac Main.java
```
