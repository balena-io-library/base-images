...
```dockerfile
FROM balenalib/cybertan-ze250-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/cybertan-ze250-debian-openjdk:latest javac Main.java
```
