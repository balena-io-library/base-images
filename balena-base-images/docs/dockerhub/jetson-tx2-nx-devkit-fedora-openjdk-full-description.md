...
```dockerfile
FROM balenalib/jetson-tx2-nx-devkit-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-tx2-nx-devkit-fedora-openjdk:latest javac Main.java
```
