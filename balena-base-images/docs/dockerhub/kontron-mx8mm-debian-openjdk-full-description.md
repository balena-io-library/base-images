...
```dockerfile
FROM balenalib/kontron-mx8mm-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/kontron-mx8mm-debian-openjdk:latest javac Main.java
```
