...
```dockerfile
FROM balenalib/stem-x86-32-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/stem-x86-32-debian-openjdk:latest javac Main.java
```
