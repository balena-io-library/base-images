...
```dockerfile
FROM balenalib/revpi-connect-s-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/revpi-connect-s-debian-openjdk:latest javac Main.java
```
