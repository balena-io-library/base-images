...
```dockerfile
FROM balenalib/jetson-orin-nano-devkit-nvme-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-orin-nano-devkit-nvme-alpine-openjdk:latest javac Main.java
```
