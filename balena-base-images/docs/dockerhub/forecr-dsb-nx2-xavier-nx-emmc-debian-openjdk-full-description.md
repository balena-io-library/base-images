...
```dockerfile
FROM balenalib/forecr-dsb-nx2-xavier-nx-emmc-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/forecr-dsb-nx2-xavier-nx-emmc-debian-openjdk:latest javac Main.java
```
