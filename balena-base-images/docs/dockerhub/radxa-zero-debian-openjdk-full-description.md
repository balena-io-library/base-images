...
```dockerfile
FROM balenalib/radxa-zero-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/radxa-zero-debian-openjdk:latest javac Main.java
```
