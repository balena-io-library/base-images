...
```dockerfile
FROM balenalib/artik533s-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/artik533s-debian-openjdk:latest javac Main.java
```
