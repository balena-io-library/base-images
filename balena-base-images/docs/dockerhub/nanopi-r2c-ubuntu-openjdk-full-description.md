...
```dockerfile
FROM balenalib/nanopi-r2c-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nanopi-r2c-ubuntu-openjdk:latest javac Main.java
```
