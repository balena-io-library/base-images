...
```dockerfile
FROM balenalib/up-squared-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/up-squared-alpine-openjdk:latest javac Main.java
```
