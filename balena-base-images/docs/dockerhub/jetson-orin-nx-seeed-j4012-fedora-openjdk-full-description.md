...
```dockerfile
FROM balenalib/jetson-orin-nx-seeed-j4012-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-orin-nx-seeed-j4012-fedora-openjdk:latest javac Main.java
```
