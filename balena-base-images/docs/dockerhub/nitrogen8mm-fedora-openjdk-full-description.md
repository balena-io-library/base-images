...
```dockerfile
FROM balenalib/nitrogen8mm-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nitrogen8mm-fedora-openjdk:latest javac Main.java
```
