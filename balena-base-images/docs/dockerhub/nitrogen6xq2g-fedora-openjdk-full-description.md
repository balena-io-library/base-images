...
```dockerfile
FROM balenalib/nitrogen6xq2g-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nitrogen6xq2g-fedora-openjdk:latest javac Main.java
```
