...
```dockerfile
FROM balenalib/phyboard-lyra-am62xx-2-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/phyboard-lyra-am62xx-2-fedora-openjdk:latest javac Main.java
```
