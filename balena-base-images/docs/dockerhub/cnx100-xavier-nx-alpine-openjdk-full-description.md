...
```dockerfile
FROM balenalib/cnx100-xavier-nx-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/cnx100-xavier-nx-alpine-openjdk:latest javac Main.java
```
