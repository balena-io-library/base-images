...
```dockerfile
FROM balenalib/imx7-var-som-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx7-var-som-alpine-openjdk:latest javac Main.java
```
