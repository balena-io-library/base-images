...
```dockerfile
FROM balenalib/raspberrypi0-2w-64-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypi0-2w-64-ubuntu-openjdk:latest javac Main.java
```
