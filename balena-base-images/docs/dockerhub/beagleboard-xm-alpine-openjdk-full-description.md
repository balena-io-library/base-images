...
```dockerfile
FROM balenalib/beagleboard-xm-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beagleboard-xm-alpine-openjdk:latest javac Main.java
```
