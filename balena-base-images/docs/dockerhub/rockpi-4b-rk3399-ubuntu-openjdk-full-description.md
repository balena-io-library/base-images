...
```dockerfile
FROM balenalib/rockpi-4b-rk3399-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/rockpi-4b-rk3399-ubuntu-openjdk:latest javac Main.java
```
