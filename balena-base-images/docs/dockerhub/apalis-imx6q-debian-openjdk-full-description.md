...
```dockerfile
FROM balenalib/apalis-imx6q-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/apalis-imx6q-debian-openjdk:latest javac Main.java
```
