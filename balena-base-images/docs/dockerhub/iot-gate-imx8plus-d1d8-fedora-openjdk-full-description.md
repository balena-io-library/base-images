...
```dockerfile
FROM balenalib/iot-gate-imx8plus-d1d8-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/iot-gate-imx8plus-d1d8-fedora-openjdk:latest javac Main.java
```
