...
```dockerfile
FROM balenalib/srd3-tx2-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/srd3-tx2-debian-openjdk:latest javac Main.java
```
