...
```dockerfile
FROM balenalib/beaglebone-fastenal-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beaglebone-fastenal-debian-openjdk:latest javac Main.java
```
