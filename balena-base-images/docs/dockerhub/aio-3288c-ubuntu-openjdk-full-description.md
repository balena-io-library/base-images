...
```dockerfile
FROM balenalib/aio-3288c-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/aio-3288c-ubuntu-openjdk:latest javac Main.java
```
