...
```dockerfile
FROM balenalib/beaglebone-pocket-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beaglebone-pocket-fedora-openjdk:latest javac Main.java
```
