...
```dockerfile
FROM balenalib/beaglebone-green-wifi-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beaglebone-green-wifi-debian-openjdk:latest javac Main.java
```
