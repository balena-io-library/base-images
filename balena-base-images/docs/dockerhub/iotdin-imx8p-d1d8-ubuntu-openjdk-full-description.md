...
```dockerfile
FROM balenalib/iotdin-imx8p-d1d8-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/iotdin-imx8p-d1d8-ubuntu-openjdk:latest javac Main.java
```
