...
```dockerfile
FROM balenalib/zc702-zynq7-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/zc702-zynq7-debian-openjdk:latest javac Main.java
```
