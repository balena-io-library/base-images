...
```dockerfile
FROM balenalib/vab820-quad-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/vab820-quad-ubuntu-openjdk:latest javac Main.java
```
