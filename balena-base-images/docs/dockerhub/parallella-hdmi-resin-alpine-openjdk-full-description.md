...
```dockerfile
FROM balenalib/parallella-hdmi-resin-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/parallella-hdmi-resin-alpine-openjdk:latest javac Main.java
```
