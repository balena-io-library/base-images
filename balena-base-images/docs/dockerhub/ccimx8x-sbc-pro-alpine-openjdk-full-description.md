...
```dockerfile
FROM balenalib/ccimx8x-sbc-pro-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/ccimx8x-sbc-pro-alpine-openjdk:latest javac Main.java
```
