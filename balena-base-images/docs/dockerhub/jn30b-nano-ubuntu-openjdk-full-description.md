...
```dockerfile
FROM balenalib/jn30b-nano-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jn30b-nano-ubuntu-openjdk:latest javac Main.java
```
