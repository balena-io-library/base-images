...
```dockerfile
FROM balenalib/photon-nano-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/photon-nano-fedora-openjdk:latest javac Main.java
```
