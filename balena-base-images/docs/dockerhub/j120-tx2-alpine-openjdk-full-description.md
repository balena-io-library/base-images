...
```dockerfile
FROM balenalib/j120-tx2-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/j120-tx2-alpine-openjdk:latest javac Main.java
```
