...
```dockerfile
FROM balenalib/jetson-orin-nano-devkit-nvme-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-orin-nano-devkit-nvme-fedora-openjdk:latest javac Main.java
```
