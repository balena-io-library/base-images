...
```dockerfile
FROM balenalib/genericx86-64-ext-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/genericx86-64-ext-alpine-openjdk:latest javac Main.java
```
