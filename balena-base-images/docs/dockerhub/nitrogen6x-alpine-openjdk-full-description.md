...
```dockerfile
FROM balenalib/nitrogen6x-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nitrogen6x-alpine-openjdk:latest javac Main.java
```
