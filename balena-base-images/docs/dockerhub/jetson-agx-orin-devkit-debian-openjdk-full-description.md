...
```dockerfile
FROM balenalib/jetson-agx-orin-devkit-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-agx-orin-devkit-debian-openjdk:latest javac Main.java
```
