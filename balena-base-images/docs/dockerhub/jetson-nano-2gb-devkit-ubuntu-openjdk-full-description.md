...
```dockerfile
FROM balenalib/jetson-nano-2gb-devkit-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-nano-2gb-devkit-ubuntu-openjdk:latest javac Main.java
```
