...
```dockerfile
FROM balenalib/nebra-hnt-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nebra-hnt-ubuntu-openjdk:latest javac Main.java
```
