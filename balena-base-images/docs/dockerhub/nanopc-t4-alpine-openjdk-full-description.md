...
```dockerfile
FROM balenalib/nanopc-t4-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nanopc-t4-alpine-openjdk:latest javac Main.java
```
