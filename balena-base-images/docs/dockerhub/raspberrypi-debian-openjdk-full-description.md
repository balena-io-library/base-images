...
```dockerfile
FROM balenalib/raspberrypi-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypi-debian-openjdk:latest javac Main.java
```
