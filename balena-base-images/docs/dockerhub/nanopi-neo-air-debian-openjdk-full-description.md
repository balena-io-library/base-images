...
```dockerfile
FROM balenalib/nanopi-neo-air-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nanopi-neo-air-debian-openjdk:latest javac Main.java
```
