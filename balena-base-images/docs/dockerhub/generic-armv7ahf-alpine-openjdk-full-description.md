...
```dockerfile
FROM balenalib/generic-armv7ahf-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/generic-armv7ahf-alpine-openjdk:latest javac Main.java
```
