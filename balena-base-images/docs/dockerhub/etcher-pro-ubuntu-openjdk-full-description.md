...
```dockerfile
FROM balenalib/etcher-pro-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/etcher-pro-ubuntu-openjdk:latest javac Main.java
```
