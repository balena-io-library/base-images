...
```dockerfile
FROM balenalib/imx8m-var-dart-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx8m-var-dart-fedora-openjdk:latest javac Main.java
```
