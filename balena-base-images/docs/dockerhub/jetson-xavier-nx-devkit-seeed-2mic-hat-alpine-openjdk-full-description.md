...
```dockerfile
FROM balenalib/jetson-xavier-nx-devkit-seeed-2mic-hat-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-xavier-nx-devkit-seeed-2mic-hat-alpine-openjdk:latest javac Main.java
```
