...
```dockerfile
FROM balenalib/raspberrypicm4-ioboard-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypicm4-ioboard-debian-openjdk:latest javac Main.java
```
