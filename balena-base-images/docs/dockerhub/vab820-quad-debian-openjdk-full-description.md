...
```dockerfile
FROM balenalib/vab820-quad-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/vab820-quad-debian-openjdk:latest javac Main.java
```
