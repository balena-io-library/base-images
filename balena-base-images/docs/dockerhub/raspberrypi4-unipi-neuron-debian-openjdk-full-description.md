...
```dockerfile
FROM balenalib/raspberrypi4-unipi-neuron-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypi4-unipi-neuron-debian-openjdk:latest javac Main.java
```
