...
```dockerfile
FROM balenalib/orange-pi-lite-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/orange-pi-lite-ubuntu-openjdk:latest javac Main.java
```
