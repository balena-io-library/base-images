...
```dockerfile
FROM balenalib/revpi-connect-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/revpi-connect-alpine-openjdk:latest javac Main.java
```
