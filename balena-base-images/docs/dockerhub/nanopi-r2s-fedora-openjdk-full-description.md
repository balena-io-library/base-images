...
```dockerfile
FROM balenalib/nanopi-r2s-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/nanopi-r2s-fedora-openjdk:latest javac Main.java
```
