...
```dockerfile
FROM balenalib/etcher-pro-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/etcher-pro-alpine-openjdk:latest javac Main.java
```
