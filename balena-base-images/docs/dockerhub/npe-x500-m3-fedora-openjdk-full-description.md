...
```dockerfile
FROM balenalib/npe-x500-m3-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/npe-x500-m3-fedora-openjdk:latest javac Main.java
```
