...
```dockerfile
FROM balenalib/firefly-rk3288-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/firefly-rk3288-fedora-openjdk:latest javac Main.java
```
