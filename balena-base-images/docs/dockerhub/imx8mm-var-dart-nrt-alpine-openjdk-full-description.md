...
```dockerfile
FROM balenalib/imx8mm-var-dart-nrt-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx8mm-var-dart-nrt-alpine-openjdk:latest javac Main.java
```
