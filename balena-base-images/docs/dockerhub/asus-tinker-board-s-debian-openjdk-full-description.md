...
```dockerfile
FROM balenalib/asus-tinker-board-s-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/asus-tinker-board-s-debian-openjdk:latest javac Main.java
```
