...
```dockerfile
FROM balenalib/raspberrypi-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberrypi-fedora-openjdk:latest javac Main.java
```
