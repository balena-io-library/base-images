...
```dockerfile
FROM balenalib/raspberry-pi-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/raspberry-pi-ubuntu-openjdk:latest javac Main.java
```
