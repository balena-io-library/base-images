...
```dockerfile
FROM balenalib/floyd-nano-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/floyd-nano-ubuntu-openjdk:latest javac Main.java
```
