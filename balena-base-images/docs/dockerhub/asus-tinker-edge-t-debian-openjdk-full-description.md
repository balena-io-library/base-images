...
```dockerfile
FROM balenalib/asus-tinker-edge-t-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/asus-tinker-edge-t-debian-openjdk:latest javac Main.java
```
