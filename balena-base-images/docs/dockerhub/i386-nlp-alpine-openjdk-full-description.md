...
```dockerfile
FROM balenalib/i386-nlp-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/i386-nlp-alpine-openjdk:latest javac Main.java
```
