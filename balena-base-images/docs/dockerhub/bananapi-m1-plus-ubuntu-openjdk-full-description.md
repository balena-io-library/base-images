...
```dockerfile
FROM balenalib/bananapi-m1-plus-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/bananapi-m1-plus-ubuntu-openjdk:latest javac Main.java
```
