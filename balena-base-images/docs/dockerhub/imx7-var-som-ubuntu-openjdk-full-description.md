...
```dockerfile
FROM balenalib/imx7-var-som-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx7-var-som-ubuntu-openjdk:latest javac Main.java
```
