...
```dockerfile
FROM balenalib/bluechiptechnology-tm3-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/bluechiptechnology-tm3-ubuntu-openjdk:latest javac Main.java
```
