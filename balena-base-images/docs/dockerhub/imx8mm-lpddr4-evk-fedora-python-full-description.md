...
```dockerfile
FROM balenalib/imx8mm-lpddr4-evk-fedora-python:latest
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx8mm-lpddr4-evk-fedora-python:latest python your-daemon-or-script.py
```
