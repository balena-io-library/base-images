...
```dockerfile
FROM balenalib/eg5120-imx8mp-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/eg5120-imx8mp-debian-openjdk:latest javac Main.java
```
