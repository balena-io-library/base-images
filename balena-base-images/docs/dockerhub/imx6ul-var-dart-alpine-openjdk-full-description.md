...
```dockerfile
FROM balenalib/imx6ul-var-dart-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx6ul-var-dart-alpine-openjdk:latest javac Main.java
```
