...
```dockerfile
FROM balenalib/jetson-tx1-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-tx1-fedora-openjdk:latest javac Main.java
```
