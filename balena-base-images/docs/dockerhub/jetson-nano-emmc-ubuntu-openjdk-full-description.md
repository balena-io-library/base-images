...
```dockerfile
FROM balenalib/jetson-nano-emmc-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/jetson-nano-emmc-ubuntu-openjdk:latest javac Main.java
```
