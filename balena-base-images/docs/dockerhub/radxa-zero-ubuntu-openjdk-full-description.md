...
```dockerfile
FROM balenalib/radxa-zero-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/radxa-zero-ubuntu-openjdk:latest javac Main.java
```
