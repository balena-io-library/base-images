...
```dockerfile
FROM balenalib/coral-dev-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/coral-dev-fedora-openjdk:latest javac Main.java
```
