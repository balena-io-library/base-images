...
```dockerfile
FROM balenalib/intel-edison-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/intel-edison-debian-openjdk:latest javac Main.java
```
