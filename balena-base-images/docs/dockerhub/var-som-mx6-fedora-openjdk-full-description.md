...
```dockerfile
FROM balenalib/var-som-mx6-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/var-som-mx6-fedora-openjdk:latest javac Main.java
```
