...
```dockerfile
FROM balenalib/orbitty-tx2-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/orbitty-tx2-debian-openjdk:latest javac Main.java
```
