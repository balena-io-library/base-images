...
```dockerfile
FROM balenalib/coral-dev-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/coral-dev-ubuntu-openjdk:latest javac Main.java
```
