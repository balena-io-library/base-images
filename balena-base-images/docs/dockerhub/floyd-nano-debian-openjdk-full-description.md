...
```dockerfile
FROM balenalib/floyd-nano-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/floyd-nano-debian-openjdk:latest javac Main.java
```
