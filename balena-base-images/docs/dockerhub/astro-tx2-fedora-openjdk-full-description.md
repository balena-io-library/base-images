...
```dockerfile
FROM balenalib/astro-tx2-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/astro-tx2-fedora-openjdk:latest javac Main.java
```
