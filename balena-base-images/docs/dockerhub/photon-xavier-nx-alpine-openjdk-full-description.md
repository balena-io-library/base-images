...
```dockerfile
FROM balenalib/photon-xavier-nx-alpine-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/photon-xavier-nx-alpine-openjdk:latest javac Main.java
```
