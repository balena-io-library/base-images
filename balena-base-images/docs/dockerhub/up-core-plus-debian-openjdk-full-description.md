...
```dockerfile
FROM balenalib/up-core-plus-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/up-core-plus-debian-openjdk:latest javac Main.java
```
