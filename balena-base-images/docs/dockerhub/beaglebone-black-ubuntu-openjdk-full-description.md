...
```dockerfile
FROM balenalib/beaglebone-black-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beaglebone-black-ubuntu-openjdk:latest javac Main.java
```
