...
```dockerfile
FROM balenalib/imx8mm-var-som-fedora-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx8mm-var-som-fedora-openjdk:latest javac Main.java
```
