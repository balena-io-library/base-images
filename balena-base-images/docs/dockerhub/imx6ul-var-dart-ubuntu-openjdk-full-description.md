...
```dockerfile
FROM balenalib/imx6ul-var-dart-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/imx6ul-var-dart-ubuntu-openjdk:latest javac Main.java
```
