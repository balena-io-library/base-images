...
```dockerfile
FROM balenalib/forecr-dsb-ornx-lan-orin-nx-16gb-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/forecr-dsb-ornx-lan-orin-nx-16gb-ubuntu-openjdk:latest javac Main.java
```
