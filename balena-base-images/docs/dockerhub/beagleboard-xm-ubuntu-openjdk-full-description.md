...
```dockerfile
FROM balenalib/beagleboard-xm-ubuntu-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/beagleboard-xm-ubuntu-openjdk:latest javac Main.java
```
