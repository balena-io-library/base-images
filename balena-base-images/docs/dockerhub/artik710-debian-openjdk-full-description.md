...
```dockerfile
FROM balenalib/artik710-debian-openjdk:latest
COPY . /usr/src/myapp
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/artik710-debian-openjdk:latest javac Main.java
```
