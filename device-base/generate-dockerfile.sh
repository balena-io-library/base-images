...
for device in $devices; do
	...
	'up-board')
		...
		fedora_baseImage='amd64-fedora'
	;;
	'via-vab820-quad'|'generic-armv7ahf'|'bananapi-m1-plus'|'orangepi-plus2')
		template='Dockerfile.tpl'
		baseImage='armv7hf-debian'
		alpine_template='Dockerfile.alpine.tpl'
		alpine_baseImage='armhf-alpine'
	...
	if [ $device != "raspberry-pi" ]; then
		for suite in $suites; do
			...
			*)
				...
				if [ $device == "intel-edison" ] || [ $device == "up-board" ]; then
					sed -e "s@#{MRAA_COMMIT}@$mraa_commit@g" \
						-e "s@#{MRAA_VERSION}@$mraa_version@g" \
						-e "s@#{UPM_COMMIT}@$upm_commit@g" \
						-e "s@#{UPM_VERSION}@$upm_version@g" Dockerfile.mraa.partial >> $debian_dockerfilePath/$suite/Dockerfile
				fi
